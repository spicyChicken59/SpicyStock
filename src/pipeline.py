...
def load_previous(docs: Path) -> dict:
    """Last night's record, for the reliability row it carries -- unless it
    is the committed fixture, whose nights never happened."""
    try:
        previous = json.loads((docs / DATA_FILE).read_text())
    except (OSError, ValueError):
        return {}
    if not isinstance(previous, dict) or previous.get("fixture"):
        return {}
    return previous

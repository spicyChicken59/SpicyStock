...
## Rule 6 is on here and off in production, deliberately

`ScanConfig.min_dollar_volume_pctile` is `0` since round 15: the liquidity
rule the pipeline applies is `universe.MIN_DOLLARS`, an absolute $5,000,000
floor that does not move with the basket, and the percentile is the one thing
`tools/make_history.py` sets differently from what a real run uses.

The gate's machinery is unchanged and the page still renders every state it
produces -- `evidence.illiquid`, the floored benchmark rung and its verdict,
and a refused row's own reason word, which no surface may collapse into a
gate rejection. Regenerated under the production default the fixture carried
none of those states, and four dashboard checks stopped being able to fail
-- a fixture that reports coverage it does not have, which is worse than one
that admits a gap. The alternative was a mutated smoke variant; this keeps
the rows real, produced by the same pipeline over the same frames.

Read the fixture's `run.liquidity.pctile` before drawing any conclusion from
it about what production refuses. `tests/test_pipeline.py`'s `rule_six`
fixture makes the same choice, for the same reason, and says so.
